...
function formulaires_login_traiter_dist($cible="",$login="",$prive=null){
	...
	$session_login = _request('var_login');
	$session_email = _request('var_login');
	$pgp = sql_allfetsel('pgp', 'spip_auteurs', "login='$session_login'");
	$pgp2 = sql_allfetsel('pgp', 'spip_auteurs', "email='$session_email'");
	if ($pgp[0]['pgp']) {
		$pgp_uns= unserialize($pgp[0]['pgp']);
	
		foreach ($pgp_uns as $key=>$val) {
			if (isset($val) && substr($key,0,4)=="quiz") {
				$GLOBALS['visiteur_session'][$key] = $val;
				ajouter_session($GLOBALS['visiteur_session']);
			}
		}
	}
	if ($pgp2[0]['pgp']) {
		$pgp_uns= unserialize($pgp2[0]['pgp']);
		foreach ($pgp_uns as $key=>$val) {
			if (isset($val) && substr($key,0,4)=="quiz") {
				$GLOBALS['visiteur_session'][$key] = $val;
				ajouter_session($GLOBALS['visiteur_session']);
