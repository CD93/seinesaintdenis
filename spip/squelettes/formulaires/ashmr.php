<?php
function formulaires_ashmr_charger_dist()
{
	$valeurs = array
	(
		'age' 			=> '',
...
function formulaires_ashmr_verifier_dist()
{
	$erreurs = array();
	...
	foreach(array('jour', 'gir') as $valeur)
		if(is_numeric(_request($valeur)))
		{
			if(_request($valeur) < 0)
				$erreurs[$valeur] = 'Vous devez entrer une valeur positive';
...
function formulaires_ashmr_traiter_dist()
{
	...
	$minimumVieillesse = 708.95; // Valeur a changer toutes les ann�es.	
	if($totalRessource < $minimumVieillesse)
	{
		$capacaiteContributive = $totalRessource - round($minimumVieillesse * 12 / 100, 0);
		if($capacaiteContributive < 0)
			$capacaiteContributive = 0;
	}
	else
		$capacaiteContributive = $totalRessource * 0.90;
	$prixJournee = _request('jour') + _request('gir');
	$montantSejour = $prixJournee * 31;
	...
	if($capacaiteContributive < $montantSejour)
		...
		$retour .= '<br />
					...
					<br />';	
	$retour .= 	'<br />
				<a href="./spip.php?article3">Autre estimation</a>
				<br />
